...
if clean_signature("../assinatura.png", "assinatura_limpa.png"):
    ...
    # Registrar as fontes TTF (TG Pagella convertida de OTF)
    pdfmetrics.registerFont(TTFont('Adventor', 'pagella-regular.ttf'))
    pdfmetrics.registerFont(TTFont('Adventor-Bold', 'pagella-bold.ttf'))
    ...
    # Textos
    c.setFont("Adventor-Bold", 12)
    c.drawCentredString(width/2, base_y - 5, nome_str)
    c.setFont("Adventor", 11)
    ...
    # Texto
    c.setFont("Adventor-Bold", 8)
    c.drawString(box_x + 140, box_y + 40, nome_str)
    c.setFont("Adventor", 8)
    c.drawString(box_x + 140, box_y + 28, data_str)
    c.showPage()
    ...
    c.setFont("Adventor", 9)
    c.drawCentredString(x_pos, y_pos, text)
    # Linha sutil
    c.setLineWidth(0.5)
    c.setStrokeColorRGB(0.5, 0.5, 0.5)
    t_width = c.stringWidth(text, "Adventor", 9)
